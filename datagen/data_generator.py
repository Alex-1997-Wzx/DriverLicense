...
''' data_generator.py
    ...
    excample: python data_generator.py --image --cut -a 100000
'''
import os
import sys
import json
...
# 是否生成英文标注信息？
parser.add_argument('--english', dest='english', action='store_true', help ='是否生成英文标注信息')
parser.add_argument('--no-english', dest='english', action='store_false', help ='是否生成英文标注信息')
...
def img_salt_pepper_noise(src, percent=0.01):
    ''' 随机加入椒盐噪声 '''
    NoiseImg = src
...
def generate_driver_card():
    ...
    # 加载不同大小的不同字体；绘制文本的时候，会随机选取一个大小的字体，丰富样本特征
    print('load fonts...')
    ...
    # 后期新增的随机字体库，在文件夹 ./material/font/random_font 中
    ...
    # templatesName存放模板文件名的列表
    templatesName = []
    # templateStrings用于存放模板实例，对于每张图片，第一次加载后会存在此处，
    # 第二次使用该模板图片就不需要再次加载了，主要为了节省时间
    templateStrings = {}
    # template_width = 1133
    # template_height = 794
    ...
    for ep in range(opt.amount):
        ...
        # 某些字字体里面没有，会变成'?',此处移除？号
        address = address.replace('?', '')
        # 若地址长度大于18，则换行（驾驶证有两行地址栏）
        if len(address) > 18:
        ...
        if opt.background:
            ...
            # 设计随机比例，将驾驶证图片嵌入背景图中
            ratio = random.randint(95, 99) / 100
