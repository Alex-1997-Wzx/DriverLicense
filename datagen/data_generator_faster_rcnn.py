...
# 输出路径？
parser.add_argument('--output', type=str, default='./gen_faster_rcnn', help="path to save generated data")
# 背景图片路径
        ...
        if opt.image:
            ...
            if opt.label:
                ...
                for key, val in items.items():
                    ...
                    # here
                    if key == 'id':
                        text_label.append(','.join((x_min, y_min, x_max, y_max, 'id', fn)))
                    elif key in ['birth', 'first', 'start', 'end']:
                        text_label.append(','.join((x_min, y_min, x_max, y_max, 'date', fn)))
                ...
                for idx, other in enumerate(others[:-2]):
                    ...
                    y_max = str(int(max(y0, y1, y2, y3)))
                    # here
                    text_label.append(','.join((x_min, y_min, x_max, y_max, other[0], fn)))
                label = '\n'.join(text_label)
